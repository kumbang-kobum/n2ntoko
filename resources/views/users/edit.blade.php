        ...
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('users.update', $user) }}" class="space-y-5">
                ...
                <div class="bg-blue-50 border border-blue-100 rounded-2xl p-4 flex items-center gap-3">
                    <div class="w-10 h-10 bg-blue-600 rounded-full flex items-center justify-center text-white font-bold shrink-0">
                    ...
                    @if($user->id === auth()->id())
                    <span class="ml-auto text-xs bg-blue-200 text-blue-800 px-2 py-1 rounded-full">Akun Saya</span>
                    @endif
                </div>
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 space-y-5">
                    ...
                    {{-- Role (disable jika edit diri sendiri) --}}
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-2">
                            Role / Jabatan <span class="text-red-500">*</span>
                            @if($user->id === auth()->id())
                            <span class="text-xs text-orange-500 font-normal ml-1">(tidak bisa mengubah role sendiri)</span>
                            @endif
                        </label>
                        <div class="grid grid-cols-2 gap-3 {{ $user->id === auth()->id() ? 'opacity-60 pointer-events-none' : '' }}">
                    ...
                    {{-- Status (disable jika edit diri sendiri) --}}
                    <div class="flex items-center justify-between py-3 px-4 bg-gray-50 rounded-xl {{ $user->id === auth()->id() ? 'opacity-60' : '' }}">
                        <div>
